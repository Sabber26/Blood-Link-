...
const LoginPage = {
  ...
  validateCsrfToken(form) {
    ...
    return !!tokenInput && !!storedToken && tokenInput.value === storedToken;
  },
  ...
  handleSubmit() {
    ...
    if (!this.validateCsrfToken(loginForm)) {
      this.showToast(
  ...
  async simulateLogin(email, password, rememberMe) {
    ...
    const lastAttempt = Number(
      sessionStorage.getItem("loginAttemptTime") || "0",
    );
    const now = Date.now();
    ...
    if (now - lastAttempt >= WINDOW_MS) {
      ...
      sessionStorage.setItem("loginAttemptTime", "0");
    }
    ...
    if (
      ...
      const nextAttempts =
        Number(sessionStorage.getItem("loginAttempts") || "0") + 1;
      sessionStorage.setItem("loginAttempts", String(nextAttempts));
      sessionStorage.setItem("loginAttemptTime", String(Date.now()));
  ...
  checkExistingSession() {
    ...
    if (isLoggedIn === "true") {
      ...
      // Redirect to home immediately if already logged in
      setTimeout(() => {
        window.location.href = "index.html";
      }, 100);
      ...
      if (formCard) {
        ...
        if (registerLink) {
          registerLink.parentNode.insertBefore(
            existingSessionDiv,
            registerLink,
          );
        }
  ...
  initSocialLogin() {
    const facebookBtn = document.querySelector(
      ".grid.grid-cols-2 button:first-child",
    );
    const googleBtn = document.querySelector(
      ".grid.grid-cols-2 button:last-child",
    );
  ...
  showFieldError(field, message) {
    ...
    errorDiv.className =
      "text-red-500 text-xs mt-1 error-message flex items-center";
    errorDiv.innerHTML = `
