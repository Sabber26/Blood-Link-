...
const Search = {
  init() {
    ...
    if (this.searchInput) {
      ...
      this.bloodGroupFilter?.addEventListener("change", () =>
        this.filterDonors(),
      );
      ...
      this.availabilityFilter?.addEventListener("change", () =>
        this.filterDonors(),
      );
    }
  },
  ...
  filterDonors() {
    ...
    const filtered = this.donors.filter((donor) => {
      const matchSearch =
        !searchTerm || donor.name.toLowerCase().includes(searchTerm);
  ...
  displayDonors(donors) {
    ...
    this.resultsContainer.innerHTML = donors
      .map(
        (donor) => `
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow">
                <div class="flex items-center space-x-4 mb-4">
                    ...
                    <div>
                        <h3 class="font-semibold text-gray-900 dark:text-white">${this.escapeHtml(donor.name)}</h3>
                        <p class="text-sm text-gray-500">${this.escapeHtml(donor.city)}, ${this.escapeHtml(donor.district)}</p>
                    </div>
                </div>
