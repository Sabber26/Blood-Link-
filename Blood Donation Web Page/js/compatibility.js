...
const CompatibilityChecker = {
    ...
    check() {
        ...
        if (isCompatible) {
            resultDiv.className = 'mt-6 p-4 rounded-lg bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200';
            resultDiv.innerHTML = `
                <div class="flex items-center space-x-2">
                    <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41L9 16.17z"/>
                    </svg>
                    <span class="font-semibold">Compatible!</span>
                </div>
            ...
            resultDiv.className = 'mt-6 p-4 rounded-lg bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-200';
            resultDiv.innerHTML = `
                <div class="flex items-center space-x-2">
                    <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/>
                    </svg>
                    <span class="font-semibold">Not Compatible</span>
                </div>
                <p class="mt-2">Donor blood group <strong>${donor}</strong> cannot donate to recipient with <strong>${recipient}</strong> blood group.</p>
            `;
        }
        
        App.showToast(isCompatible ? 'Blood groups are compatible!' : 'Blood groups are not compatible', 
