...
const RegisterPage = {
  ...
  initCsrfProtection() {
    ...
    if (!registerForm.querySelector('input[name="csrf_token"]')) {
  ...
  handleSubmit() {
    const registerForm = document.getElementById("register-form");
    if (!this.validateCsrfToken(registerForm)) {
      this.showToast(
    ...
    if (
      !this.validateSelect(
        document.getElementById("blood-group"),
        "Blood group",
      )
    ) {
      isValid = false;
  ...
  async simulateRegistration() {
    ...
    const userData = {
      ...
      availability: document.querySelector('input[name="availability"]:checked')
        ?.value,
